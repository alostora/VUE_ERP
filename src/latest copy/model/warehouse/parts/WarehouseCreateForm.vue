<template>
  <div class="warehouse-create-form">
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        const url = `${general_request.BASE_URL}/admin/company/branch/warehouse`;
        const response = await this.$http.post(url, payload, {
          headers: general_request.headers,
        });

        this.resetForm();
        this.$emit("warehouse-created", response.data.data);

        this.showToast(
          "success",
          this.$t("warehouses.success"),
          this.$t("warehouses.warehouseCreated")
        );
      } catch (error) {
    ...
    handleGenericError(responseData) {
      this.error = responseData.message || this.$t("warehouses.createError");
    },
