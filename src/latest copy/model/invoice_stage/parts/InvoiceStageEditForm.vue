<template>
  <div class="invoice-stage-edit-form">
      ...
      <div class="grid">
        ...
        <div class="col-12 md:col-6 field">
          ...
          <InputText
            ...
            class="w-full"
            :placeholder="$t('invoice_stages.namePlaceholder')"
          />
        ...
        <div class="col-12 md:col-6 field">
          ...
          <InputText
            ...
            class="w-full"
            :placeholder="$t('invoice_stages.nameArPlaceholder')"
          />
      ...
      <div class="grid">
        <div class="col-12 md:col-6 field">
          ...
          <Textarea
            ...
            rows="3"
            class="w-full"
            :placeholder="$t('invoice_stages.detailsPlaceholder')"
          />
        </div>
        ...
        <div class="col-12 md:col-6 field">
          ...
          <Textarea
            ...
            rows="3"
            class="w-full"
            :placeholder="$t('invoice_stages.detailsArPlaceholder')"
          />
        </div>
      </div>

      <!-- Is Default (Read-only if already default) -->
      <div class="field mb-4">
        <div class="flex align-items-center">
          <Checkbox
            ...
            :binary="true"
            class="mr-2"
            :disabled="invoiceStage.is_default"
          />
          <label for="is_default" class="font-medium">
            {{ $t("invoice_stages.isDefaultLabel") }}
            <span v-if="invoiceStage.is_default" class="text-green-600 ml-2">
              ({{ $t("invoice_stages.defaultStage") }})
            </span>
          </label>
        </div>
        <small class="p-text-secondary" v-if="invoiceStage.is_default">
          This is the current default stage. Use "Set as Default" action in the
          table to change it.
        </small>
        <small class="p-text-secondary" v-else>
          Check to set this stage as the default for new invoices
        </small>
      </div>
...
export default {
  ...
  watch: {
    invoiceStage: {
      immediate: true,
      deep: true,
      handler(newInvoiceStage) {
        if (newInvoiceStage && newInvoiceStage.id) {
          this.populateForm(newInvoiceStage);
        } else {
          this.resetForm();
        }
      },
  ...
  methods: {
    populateForm(invoiceStage) {
      this.formData = {
        id: invoiceStage.id || "",
        name: invoiceStage.name || "",
        name_ar: invoiceStage.name_ar || "",
        details: invoiceStage.details || "",
        details_ar: invoiceStage.details_ar || "",
        is_default: invoiceStage.is_default || false,
      };
    },
    resetForm() {
      this.formData = {
    ...
    async submitForm() {
      ...
      try {
        ...
        const url = `${general_request.BASE_URL}/admin/company/invoice-stage/${this.formData.id}`;
        const response = await this.$http.patch(url, payload, {
          headers: general_request.headers,
        });

        this.$emit("invoice-stage-updated", response.data.data);

        this.showToast(
          "success",
          this.$t("invoice_stages.success"),
          this.$t("invoice_stages.invoiceStageUpdated")
        );
      } catch (error) {
    ...
    handleGenericError(responseData) {
      this.error =
        responseData.message || this.$t("invoice_stages.updateError");
    },
