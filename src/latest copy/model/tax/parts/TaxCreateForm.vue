  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        const url = `${general_request.BASE_URL}/admin/company/tax`;
        const response = await this.$http.post(url, payload, {
          headers: general_request.headers,
        });

        this.resetForm();
        this.$emit("tax-created", response.data.data);

        this.showToast(
          "success",
          this.$t("taxes.success"),
          this.$t("taxes.taxCreated")
        );
      } catch (error) {
    ...
    handleGenericError(responseData) {
      this.error = responseData.message || this.$t("taxes.createError");
    },
