...
export default {
  ...
  methods: {
    ...
    closeModal() {
      this.visible = false;
      this.form = { phone: "", is_default: false };
    },
    async submit() {
      try {
        // POST to collection endpoint with contact_id in body (per CONTACT_PROMPET.md)
        await this.$http.post(
          `${general_request.BASE_URL}/admin/company/contact-phone`,
          {
        ...
        this.$toast.add({
          ...
          detail: this.$t("contacts.phoneCreated"),
        });
        this.$emit("created");
        this.closeModal();
      } catch (err) {
