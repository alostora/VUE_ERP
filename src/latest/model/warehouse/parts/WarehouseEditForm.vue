      ...
      <div class="grid">
        ...
        <div class="col-12 md:col-6 field">
          ...
          <InputText
            ...
            class="w-full"
            :placeholder="$t('warehouses.nameArPlaceholder')"
          />
      ...
      <div class="grid">
        <div class="col-12 md:col-6 field">
          ...
          <Textarea
            ...
            rows="3"
            class="w-full"
            :placeholder="$t('warehouses.detailsPlaceholder')"
          />
        </div>

        <div class="col-12 md:col-6 field">
          <label for="details_ar" class="font-bold block mb-2">
            {{ $t("warehouses.details_ar") }}
          </label>
          <Textarea
            id="details_ar"
            ...
            rows="3"
            class="w-full"
            :placeholder="$t('warehouses.detailsArPlaceholder')"
          />
        </div>
      </div>

      <!-- Is Default (DISABLED when currently default) -->
      <div class="field mb-3">
        <div class="flex align-items-center">
          ...
          <label for="is_default" class="font-medium">
            ...
            <span v-if="isCurrentlyDefault" class="text-green-500 ml-2">
              ({{ $t("warehouses.currentlyDefault") }})
            </span>
          </label>
        </div>
        <small class="p-text-secondary">
          {{ $t("warehouses.defaultWarehouseHint") }}
...
export default {
  ...
  computed: {
    ...
    // Check if the current warehouse is the default one
    isCurrentlyDefault() {
      return this.warehouse.is_default === true;
    },
  },
  data() {
    return {
  ...
  methods: {
    populateForm(warehouse) {
      this.formData = {
        ...
        details_ar: warehouse.details_ar || "",
        // Force is_default to true if this is the current default warehouse
        is_default: this.isCurrentlyDefault
          ? true
    ...
    validateForm() {
      ...
      if (!this.formData.name?.trim()) {
        this.errors.name = this.$t("warehouses.nameRequired");
      }

      if (!this.formData.name_ar?.trim()) {
        this.errors.name_ar = this.$t("warehouses.nameArRequired");
      }
    ...
    async submitForm() {
      ...
      try {
        const payload = {
          ...
          // If this is the current default warehouse, always send is_default as true
          is_default: this.isCurrentlyDefault ? true : this.formData.is_default,
        };

        const url = `${general_request.BASE_URL}/admin/company/branch/warehouse/${this.formData.id}`;
        const response = await this.$http.patch(url, payload, {
          headers: general_request.headers,
        });

        this.$emit("warehouse-updated", response.data.data);

        this.showToast(
          "success",
          this.$t("warehouses.success"),
          this.$t("warehouses.warehouseUpdated")
        );
      } catch (error) {
    ...
    handleGenericError(responseData) {
      this.error = responseData.message || this.$t("warehouses.updateError");
    },
    handleNetworkError(error) {
      this.error = error.message || this.$t("warehouses.networkError");
    },
